...
*Пришлите скриншоты со скриптами и демонстрацией их работы*
 ...
 - количестве процессов для каждого пользователя;
   ```ps aux | awk 'NR>1{total[$1]++;} END{for(id in total)print id,total[id])'```
   ![5-3-3](./hw-5-3/5-3-3-1.png)
 - процессе с самым большим PID;
   ```ps aux | sort -rnk2 | awk '{print $0}' | head -1```
   ![5-3-3-2](./hw-5-3/5-3-3-2.png)
 - суммарном использовании памяти различными пользователями.
   ```ps aux | awk '{s += $4} END "print %MEM:" s}'```
   ![5-3-3-3](./hw-5-3/5-3-3-3.png)
